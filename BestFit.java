...
public class BestFit extends MemoryAllocationAlgorithm {
    ...
    public int fitProcess(Process p, ArrayList<MemorySlot> currentlyUsedMemorySlots) {
        ...
        if(!availableBlocks.isEmpty()) {
            int minIndex=0, minAvailableMemory = availableBlocks.get(0);
            for (int y = 0; y < availableBlocks.size(); y += 2) {
                if (availableBlocks.get(y) < minAvailableMemory) {
                    minAvailableMemory = availableBlocks.get(y);
                    minIndex=y;
                }
            }
            address=availableBlocks.get(minIndex+1);
        }
        return address;
    }
